...
contract LiquidationOperator2 is IUniswapV2Callee {
    ...
    ILendingPool constant lendingPool = ILendingPool(0x7d2768dE32b0b80b7a3454c06BdAc94A69DDc7A9); //AAVE liquidate target user


    address constant liquidationTarget = 0x59CE4a2AC5bC3f5F225439B2993b86B42f6d3e9F; //Target user
    uint debt_USDT;
    ...
    constructor() {
        ...
        // debt_USDT = 2000000000;//2000.000000
    ...
    function operate() external {
        ...
        uniswapV2Pair_WBTC_USDT.swap(0, debt_USDT, address(this), "$"); //flash loan
    ...
    function uniswapV2Call(
        ...
        (uint256 reserve_WETH_Pool1, uint256 reserve_USDT_Pool1, ) = uniswapV2Pair_WBTC_USDT.getReserves(); // Pool1
        ...
        USDT.approve(address(lendingPool), debtToCover); //มอบอำนาจให้ aave
        lendingPool.liquidationCall(address(WBTC), address(USDT), liquidationTarget, debtToCover, false);
        uint collateral_WBTC = WBTC.balanceOf(address(this)); //check WBTC ว่าได้จริงๆนะ
        console.log("collateral_WBTC = ", collateral_WBTC); //ด๔ว่าได้ WBTC มาเท่าไร


        // 2.2 repay
        console.log('--------------- Repay flash loan USDT -------------------------');
        uint repay_WBTC = getAmountIn(debtToCover, reserve_WETH_Pool1, reserve_USDT_Pool1);
        console.log('WBTC to repay: ',repay_WBTC); //ต้องจ่ายคืนเท่าไร
        WBTC.transfer(address(uniswapV2Pair_WBTC_USDT), repay_WBTC);

        // 2.3 swap WBTC to WETH
        console.log('--------------- swap WBTC to WETH -------------------------');
        uint256 swap_WBTC_to_WETH = WBTC.balanceOf(address(this));
        console.log('WBTC Remaining: ',swap_WBTC_to_WETH);
        WBTC.transfer(address(uniswapV2Pair_WBTC_WETH), swap_WBTC_to_WETH);
        uint amountOut_WETH = getAmountOut(swap_WBTC_to_WETH, reserve_WBTC_Pool2, reserve_WETH_Pool2);
        uniswapV2Pair_WBTC_WETH.swap(0, amountOut_WETH, address(this), ""); //swap
