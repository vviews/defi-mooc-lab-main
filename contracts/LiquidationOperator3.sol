...
contract LiquidationOperator3 is IUniswapV2Callee {
    ...
    function operate() external {
        ...
        uniswapV2Pair_WETH_USDC.swap(debt_USDC,0, address(this), "$"); //flash loan
    ...
    function uniswapV2Call(
        ...
        USDC.approve(address(lendingPool), debtToCover); //มอบอำนาจให้ aave
        lendingPool.liquidationCall(address(WETH), address(USDC), liquidationTarget, debtToCover, false);
        uint collateral_WETH = WETH.balanceOf(address(this)); //check WETH ว่าได้จริงๆนะ
        console.log("collateral_WETH = ", collateral_WETH); //ด๔ว่าได้ WETH มาเท่าไร


        // 2.2 repay
        console.log('--------------- Repay flash loan USDT -------------------------');
        uint repay_WETH = getAmountIn(debtToCover, reserve_WETH_Pool1, reserve_USDC_Pool1);
        console.log('WETH to repay: ',repay_WETH); //ต้องจ่ายคืนเท่าไร
